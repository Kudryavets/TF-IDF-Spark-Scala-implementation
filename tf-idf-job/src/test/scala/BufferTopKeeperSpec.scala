...
class BufferTopKeeperSpec extends WordSpec with Matchers {
  "BufferTopHolder" should {
    "addElement correctly" when {
      "buffer is empty" in {
        val buffer = new BufferTopKeeper(3)
    ...
    "merge another bufferCorrectly" in {
      var bufferLft = new BufferTopKeeper(3)
      bufferLft = (1 to 4)
        .map(i => (s"some_word_$i", i/2.0))
        .foldLeft(bufferLft)((acc, el) => bufferLft.addElement(el))
  
      var bufferRght = new BufferTopKeeper(3)
      bufferRght = (5 to 6)
        .map(i => (s"some_word_$i", i/2.0))
